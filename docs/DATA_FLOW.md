...
3. The UI dispatches `fetchAndSetStudents()` / `refreshCurrentView()` (store action) which:
  ...
  - calls `servers/frontend/src/lib/api-client.ts::getFilteredStudents(filters)`
...
- WebSocket is used only for notifications and small RPCs (non-snapshot ops). The single, central socket is owned by `servers/frontend/src/app/page.tsx`.
...
- `servers/frontend/src/app/page.tsx` — central WS + visibility handling and debounced data_changed handler.
- `servers/frontend/src/lib/api-client.ts` — `getFilteredStudents()` and `getStudentById()` use HTTP.
- `servers/frontend/src/hooks/use-student-store.ts` — `fetchAndSetStudents()`, `refreshCurrentView()`, `applyRealtimeUpdate()` implement snapshot fetch + merge logic.
- `servers/frontend/src/app/actions.ts` — action wrappers used by UI components.
...
Docs & follow-ups
 - This file documents the expected flow; update `docs/QUICKSTART.md` to reflect runtime startup order (start backend first, then frontend) and note that WS connect is centralized in `servers/frontend/src/app/page.tsx`.
 - This file documents the expected flow; update `docs/QUICKSTART.md` to reflect runtime startup order (start backend first, then frontend) and note that WS connect is centralized in `servers/frontend/src/app/page.tsx`.
