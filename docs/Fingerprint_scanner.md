...
- Purpose: allow fingerprint scanners (or other automated devices) to submit `scan_student` events without full user authentication.
- Configuration: set an environment variable `SCANNER_TOKEN` on the backend host.

Example (Linux/macOS):

```bash
# from the machine that will run the backend
export SCANNER_TOKEN="your-secret-token-here"
# Start the backend using the canonical startup steps in docs/QUICKSTART.md
```
...
- If `SCANNER_TOKEN` is not set, unauthenticated scans are rejected.
...
- Protect `SCANNER_TOKEN`; treat it like a secret key and rotate it if compromised.
